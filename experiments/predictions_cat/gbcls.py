...
""",
    ...
    train_parser.add_argument(
        "--cols", help="CSV columns for data (x)", nargs="+", required=True,
    )
    ...
    train_parser.add_argument("--cv", help="GridSearchCV", action="store_true")
...
def classify_csv(
    ...
    data_encoders = model_dict["data_encoders"]
    for col in data_cols:
        x.loc[:, col] = data_encoders[col].transform(x[col])

    label_encoder = model_dict["le"]

    if scores:
        p = model.predict_proba(x)
        p_scores = np.max(p, axis=1).tolist()
        p_labels = np.argmax(p, axis=1).tolist()
        p_labels = label_encoder.inverse_transform(p_labels)
